...
## 2026-08-xx | 설명문의 마크다운 기호 처리

- **사건**: 36종 description_ko 원문에 마크다운 강조(**)가 포함되어 있었으나,
  "문구를 수정하지 말라"는 지시에 따라 에이전트가 그대로 저장하고 판단을 요청
- **결정**: DB에는 순수 텍스트만. ** 제거
- **사유**: DESIGN.md에 본문 내 굵은 글씨 규격이 없음(body-serif 단일 웨이트).
  마크다운 파서 추가는 불필요한 의존성
- **원칙**: 문서용 강조 표기와 DB 저장값을 구분한다. 
  문서에는 남기되 시드 시 벗겨낸다

  ## 2026-08-xx | AI 서술자의 경계 재정의

- **사건**: Part 3-7의 "평가·감상 금지"가 과하게 좁게 해석되어,
  AI가 감정을 짚어주는 것조차 막는 상태였다.
- **정정**: 금지 대상은 **판정**이지 서술이 아니다.
  "오늘 두 분 정말 다정하셨네요"는 허용된다.
  "이날은 세준이 사과했어야 했다"는 금지된다.
- **판단 기준**: 두 사람이 각자 읽었을 때 한쪽이라도 불편해질 여지가 있는가.
- **연애리그 특칙**: 스탯 하락 해설은 원인을 외부 상황에 귀속시킨다.
  개인의 태도를 지목하지 않는다.

## 2026-08-xx | AI 매거진 메이커 캐릭터화 폐기

- **결정**: AI 서술자를 캐릭터화하지 않는다
- **사유**: 캐릭터화하면 관계가 생기고, 관계가 생기면 말을 걸게 되고,
  말을 걸면 판정하게 된다. 발행물 실물에도 화자가 등장하지 않으며,
  목차 문장의 주어는 인격체가 아니라 "온도록"이라는 서비스명이다.
- **대체**: 민감도는 정직한 고지로 다룬다
  (화면 6 AI 활용 동의, 발행물 고지 문구)

## D-{번호} tsconfig types 화이트리스트 전환 (2026-09-02)

**결정**: `tsconfig.json` compilerOptions에 `"types": ["jest", "node"]` 추가.

**배경**: Phase 2부터 `npx tsc --noEmit`이 720건 실패 상태로 방치됐다.
표준 검증 명령 하나가 그동안 무의미했다.

**원인 확정 과정**: `--showConfig`로 extends 병합 후에도 `types`/`typeRoots`가
부재함을 확인. `@types/jest`는 루트 node_modules에 정상 설치. `--types jest`로
720 → 15로 감소했으나 잔여 15건이 baseline의 node 관련 에러와 일치.
`--types jest,node`에서 0건. 자동 @types 포함 메커니즘이 특정 패키지가 아니라
통째로 동작하지 않는 상태였다.

**근본 원인 미규명**: `typeRoots` 부재·호이스팅 정상인데 자동 스캔이 실패하는
이유는 밝히지 못했다. 이 설정은 우회책이다.

**영향 (중요)**: `types`가 화이트리스트가 되었다. 앞으로 전역 타입 패키지를
추가하면 이 배열에 직접 등록해야 한다. **누락 시 조용히 미인식된다.**
`import`로 끌어오는 타입은 영향받지 않는다.

**절대 규칙 8**: 지시받은 설정 파일 변경. 변경 전 `types` 키 부재 → 변경 후
`["jest", "node"]`.