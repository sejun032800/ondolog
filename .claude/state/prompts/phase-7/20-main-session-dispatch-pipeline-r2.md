...
> 보관 경로: `.claude/state/prompts/phase-7/20-main-session-dispatch-pipeline-r2.md`
> 작성: 프롬프트 엔지니어 세션 / 2026-09-10
>
> **r2 — 위임 전 개정.** 원본은 접미사 없는 파일로 병렬 보관한다.
> 원본이 `app_config` 조회 모듈을 `configLookup.ts`로 적었으나 `#12`가
> 확정한 계약은 `coeffLookup.ts`였다. `HANDOFF.md` 보고 서술을 옮겨
> 적는 과정에서 어긋났고, **정적 규칙 D가 판정에 쓰는 상수가
> 원본**이라는 규정(r21)의 근거가 된 자리다. 그대로 위임했다면
> 에이전트가 만든 파이프라인이 자기 규칙 D에 걸렸다.
>
> r2 변경: 모듈명 `coeffLookup.ts` · 리비전 r21 · 착수 조건 ①을
> 최소 파일 + `tsc` 실행으로 실증 · 0단계 파일명 확인을 저장소 전체로
>
> 짝: `20-engine-dev-pipeline-r2.md`
...
- [ ] **저장소 전체에** 공백·괄호가 들어간 파일명이 있는가 →
      `git mv`로 정리하고 커밋
- [ ] 이번 위임 파일이 `20-engine-dev-pipeline-r2.md`로 있는가
...
| `.claude/state/prompts/phase-7/20-engine-dev-pipeline-r2.md` | **이번에 위임할 프롬프트 원문** |
...
```powershell
Select-String -Path docs/ONDOLOG_MASTER.md -Pattern 'DOC_REVISION: 2026-09-10-r21'
```
...
에이전트가 1부에서 확인하지만, **실패가 확실하면 위임 자체가
낭비입니다.**

```powershell
Select-String -Path tsconfig.json -Pattern 'include|exclude' -Context 0,6
Select-String -Path docs/ONDOLOG_SCHEMA.md -Pattern 'skip_reason|generation_attempts|corners' -Context 0,4
```

- [ ] `tsconfig.json`이 `supabase/functions/`를 포함하는가
- [ ] `corners` 테이블의 컬럼 구성 (참고용 기록)

**`tsconfig.json`이 명백히 범위 밖이면 위임하지 말고 보고하십시오.**

> **`include`에 잡히는 것만으로 통과 판정하지 마십시오.** Deno 코드가
> Node 설정 아래에서 컴파일되는지는 별개이고, 지금 에러가 없는 것은
> 디렉터리가 비어 있기 때문입니다. **에이전트가 1부에서 최소 파일로
> 실제 확인합니다.**

> `corners` 테이블은 이미 존재하며 마이그레이션이 필요 없습니다.
> 착수 조건 ②는 **필드 매핑 확인**이지 구현 금지 사유가 아닙니다.
...
**2-4·2-5에서 찾은 것을 알려주지 마세요.** 1부와 착수 전 보고가 그것을
직접 확인하는 절차입니다.
...
.claude/state/prompts/phase-7/20-engine-dev-pipeline-r2.md 를
...
- [ ] 둘 다 확인·보고됐는가
- [ ] **①을 `tsconfig` 읽기가 아니라 최소 파일 + `tsc` 실행으로
      확인했는가**
- [ ] 확인용 파일이 **지워졌는가**
...
```powershell
git status --porcelain -uall tsconfig.json supabase/migrations
```

- [ ] **출력이 없어야 정상**입니다. 둘 다 이번 범위 밖입니다
...
## 착수 조건 (최우선)
- ① Edge Function이 tsc 통과: {O/X} — 확인 방법 {최소 파일 실험 / tsconfig만}
- 확인용 파일 삭제: {O/X}
- ② 필드 매핑 17-0-5-B 일치: {일치 / 어긋남 — 내용}
- ①이 실패했는데 구현 진행: {아니오 / 예 — 보고}
- tsconfig.json · migrations 변경: {없음 / 있음 — 위반}
...
- **2-4·2-5에서 찾은 것을 알려주는 것**
- **착수 조건이 실패했는데 진행시키는 것**
- **`tsconfig.json`을 고치는 것** — 별도 지시가 필요합니다
