...
> (개정 — 2-3 문서 최신성 검사 교체, 2-5 명령 오류 수정, 엔진 버전 맵 반영)
>
> 짝: `02-engine-dev-norm-enumeration.md`
...
```powershell
Select-String -Path docs/ONDOLOG_MASTER.md -Pattern '10-1-3|10-8-1|10-8-2|10-8-3|17-3'
```
...
> 아래 세 문자열은 **2026-09-02 갱신본에만 존재**합니다. 옛 판에도 있는
> 표현으로 검사하면 뒤처진 문서가 통과해버립니다(실제로 한 번 발생했습니다).

```powershell
Select-String -Path docs/ONDOLOG_MASTER.md -Pattern '산술평균은 백분위'
Select-String -Path docs/ONDOLOG_MASTER.md -Pattern '정적 import하지 않는다'
Select-String -Path docs/ONDOLOG_MASTER.md -Pattern '생성 시점의 엔진 코드 버전'
```

**세 개 모두 매치가 나와야 합니다.** 하나라도 없으면 저장소의 MASTER가
갱신 전 판입니다. **문서에 문구를 추가하지 말고**, 최신본으로 교체해야 한다는
사실을 보고하고 멈추세요 — 이 파일은 마스터 PM 세션에서 관리됩니다.

### 2-4. 기존 채점 함수가 실제로 존재하는가

```powershell
Get-ChildItem src/engine -Recurse -File -Include *.ts | Select-String -Pattern 'Enneagram|Attachment|Sternberg|Big5|SixStats' -List
```

**파일 경로와 함수명을 보고에 포함하세요.** 전혀 없으면 멈추고 보고하세요 —
에이전트가 채점을 새로 짜게 되고, 그게 이 작업에서 가장 피해야 할 결과입니다.

### 2-5. 엔진 버전 상수가 존재하는가

프롬프트가 `engineVersions`를 **기존 상수에서 읽으라**고 지시합니다.

```powershell
Get-ChildItem src -Recurse -File -Include *.ts,*.tsx | Select-String -Pattern 'ENGINE_VERSION'
```

> `Select-String`에는 `-Recurse`가 없습니다. 반드시 `Get-ChildItem`으로
> 파일을 넘겨 파이프하세요.

**찾은 상수명·값·파일 경로를 전부 보고에 포함하세요.** 모듈별로 여러 개
존재하는 것이 정상입니다 — 프롬프트가 맵으로 기록하도록 지시합니다.
하나도 없으면 보고하고 멈추세요.
...
JSON 구조, 백분위 조회 방식, 스크립트 배치는 에이전트 판단 영역입니다.

**분산 지배 점검과 에니어그램 분포에 대해 해석을 주지 마세요.**
...
확인 후, 문서가 지시한 대로 기존 채점 함수와 엔진 버전 상수를 먼저 찾아
...
```powershell
Get-ChildItem src/engine -Recurse -File -Include *.ts | Select-String -Pattern "norm-synthetic|data/norm"
```

**매치가 나오면 위반입니다.** 백분위 조회 함수의 시그니처를 직접 열어
**규준 데이터가 인자로 들어오는지** 확인하세요.
...
- `engineVersions`가 **맵**인가. 열거가 import한 모듈이 전부 들어 있고,
  각 값이 **2-5에서 찾은 상수의 값과 일치**하는가
...
**존재만 확인하지 말고 작동을 확인하세요.** 통과하도록만 쓰여 있으면 장치 전체가
무의미해집니다(`#1`의 throw 기대 테스트와 같은 함정입니다).
...
- 테스트: **267 대비 감소가 없어야 합니다.** 깨졌다면 `git diff __tests__/`로
  에이전트가 고쳐서 통과시켰는지 확인하세요. **기존 테스트 파일 수정은 금지**입니다
- tsc: **0 에러**
- `UNRESOLVED`: **여전히 3건이어야 합니다.** 줄었으면 `leagueStats.shrinkage`를
  임의로 해소한 것이므로 보고하세요
...
## 사전 점검
- 작업 트리: {clean / 변경 N건}
- 참조 절 존재: 10-1-3 {O/X} / 10-8-1 {O/X} / 10-8-2 {O/X} / 10-8-3 {O/X} / 17-3 {O/X}
- MASTER 최신성 3종: 산술평균 위치 {O/X} / 정적 import 금지 {O/X} / 생성 시점 엔진 버전 {O/X}
- 기존 채점 함수: {경로와 함수명}
- 엔진 버전 상수: {상수명 / 값 / 경로 전부}
...
- 배열 7개 길이: {각각}
- version: {값}
- engineVersions 맵: {전체} — 2-5 상수와 일치 {O/X}
- 에니어그램 9종 요약: {있음/없음}
- 재생성 해시 일치: {일치/불일치}
...
- **`docs/ONDOLOG_MASTER.md`를 편집하는 것** — 2-3이 실패하면 문구를 넣지 말고
  최신본 교체가 필요하다고 보고하세요. 이 파일은 마스터 PM 세션에서 관리됩니다
- 분산 지배나 에니어그램 분포 수치를 **당신이 해석하는 것** — 수치만 옮기세요
- OVR 파이프라인을 이번 기회에 배선하는 것 — 별도 작업입니다
- 엔진 버전 상수가 없을 때 **새로 만드는 것** — 보고하고 멈추세요
- 커밋·푸시 (사람이 실행합니다)
- 지시받지 않은 설정 파일 변경
