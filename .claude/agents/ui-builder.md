...
# `.claude/agents/ui-builder.md` 추가분

> MASTER에서 분리했다. 에이전트 정의 파일 끝에 이어 붙인다.
> (기존에는 MASTER Part 9-4 본문 안에 있었으나, 위치가 잘못되어 정리했다)
...
- 온디바이스 처리 고지 문구는 자리표시자로 두되, 4가지 요지를 반드시 담는다 
  (온디바이스 처리 / 원본만 서버 저장 / 언제든 철회 가능 / 거부해도 다른 기능 이용 가능)
- 동의 없이 얼굴 인식 코드 경로에 진입하지 않는다
- 철회 시 로컬 얼굴 데이터 삭제는 자동 검증이 불가능하므로
  QA 체크리스트에 수동 항목으로 별도 명시할 것

# Phase 10 전용 주의사항 (매거진 제작 탭)
- Phase 4의 매거진 탭은 당시 규격대로 완료됐다. 되돌리는 게 아니라 확장한다
- 참조: MASTER Part 9-5 (실루엣·진행률·제작 화면) + DESIGN §13-5
- 진행률 바에 애니메이션을 넣지 않는다. 로딩이 아니라 상태 표시다
- "대기 중"을 액센트색으로 강조하지 않는다. 액센트는 온도·지표 전용(DESIGN §1-1)
```