...
| `docs/ONDOLOG_DESIGN.md` | 디자인 시스템 (색·서체·간격·화면 구성) |
...
|---|---|---|---|
| 0 | 프로젝트 셋업 | 메인 세션 | ✅ 완료 |
| 1 | DB 마이그레이션 + RLS | `db-architect` | ✅ 완료 |
| 2 | 엔진 (채점·스탯·온도·DNA) | `engine-dev` | ✅ 완료 |
| 3 | 온보딩 8화면 + 카카오 로그인 | `ui-builder` | ✅ 완료 |
| — | **디자인 시스템 적용** | `ui-builder` | ✅ 완료 |
| 4 | 5개 탭 UI + 게이팅 | `ui-builder` | ✅ 완료 |
| 5 | 채팅 실시간 + 큐 영속화 | `ui-builder` | ✅ 완료 |
| **6** | **피드 + 얼굴 인식** | **메인 세션** | 🔵 진행 중 (3/8단계) |
| 7 | 코너 생성 파이프라인 | `corner-pipeline` | 대기 |
| 8 | PDF 조판 + 뷰어 | `corner-pipeline` + 메인 | 대기 |
| 9 | 구독·결제 | `ui-builder` | 대기 |
| **10** | **매거진 제작 탭** (실루엣 + 제작 진행률) | `ui-builder` | 대기 — **Phase 2 확장의 선행 조건** |

### ⚠️ 최대 리스크 — 실기기 검증 누적

**Phase 0 이후 실기기 검증이 없다.** 안드로이드 폰 부재로 재빌드를 미뤄왔고, Phase 1~6이 전부 미검증 상태로 쌓였다.

**재빌드 대기 목록**

| 항목 | 출처 |
|---|---|
| `expo-web-browser`, `expo-linking` | Phase 3 (카카오 로그인) |
| 폰트 5종 (MaruBuri×3, Pretendard×2) | 디자인 시스템 |
| 얼굴 인식 SDK | Phase 6 |
| `@react-native-kakao/map` | Phase 6 |
| `expo-image-picker` | Phase 6 (대표사진 플레이스홀더 해소) |

재빌드 후 **5가지가 동시에 처음 검증된다.** 문제 발생 시 원인 범위가 넓다.

**대응**: 재빌드 직후 Phase 1→5 순서대로 하나씩 검증하고 Phase 6으로 넘어간다.
...
| 단계 | 작업 | 담당 | 상태 |
|---|---|---|---|
| 1 | 생체정보 동의 흐름 (화면 A) | `ui-builder` | ✅ |
| 2 | SDK 설치 + Dev Build 재빌드 | 메인 | ⏸ 폰 대기 |
| 3 | 대표사진 등록 화면 | `ui-builder` | ✅ (플레이스홀더 피커) |
| 4 | 탐지·임베딩 서비스 래퍼 | 메인 | ⬜ |
| 5 | 매칭 순수 함수 (`faceMatch.ts`) | `engine-dev` | ✅ |
| 6 | 라이브러리 스캔 파이프라인 | 메인 | ⬜ |
| 7 | EXIF 클러스터링 → `dates`/`date_stops` | 메인 | ⬜ |
| 8 | AI 데이트 서사 생성 | `corner-pipeline` 패턴 | ⬜ |

**3·5번은 서브에이전트 위임 가능** — 순수 UI, 순수 계산이라 반복 디버깅이 없다.
**2·4·6·7번은 메인 세션** — 네이티브 빌드 에러와 실기기 테스트가 반복된다.

#### 얼굴 인식 아키텍처 — 3단계

```
1단계 탐지    → 사진 속 얼굴 위치 (네이티브 SDK)
2단계 임베딩  → 얼굴을 숫자 벡터로 (네이티브 SDK)
3단계 매칭    → 벡터 유사도 비교 (순수 함수) ← 라이브러리 의존 없음
```

**1차 채택**: `react-native-expo-facial-recognition` (ArcFace ONNX, 온디바이스)
**교체 트리거**: 오탐지율 체감 / 특정 OS 크래시 / 6개월 방치 / 저사양 기기 성능

> **cloud sync 옵션을 활성화하지 않는다.** 로컬 저장 모드로만 사용한다 (절대 규칙 1).
> 매칭 로직을 `src/engine/faceMatch.ts`로 분리했으므로, 라이브러리를 교체해도 이 함수는 유지된다.

**선행 필요**: ~~생체정보 동의 UI~~ → 2026-08-27 화면 A 구현 완료. **법무 문구는 여전히 자리표시자.**
...
**선행 필요**
- ~~AI 매거진 메이커 캐릭터 설계~~ → **폐기.** 캐릭터화하지 않는다 (MASTER Part 9-3)
- LLM 한국어 품질 실측 비교 (Sonnet 5 기준, 대안과 대조)
- ~~연애 온도 결합 공식~~ → **해소** (MASTER Part 10-7)
- ~~OVR 포지션 가중치 + 규준집단 + 베이지안 사전평균~~ → **해소** (MASTER Part 10-8)
- **DNA base_score 궁합 공식** — 미해소. 10-6 매트릭스가 범주형이라 범주→점수 변환 규칙 필요
- `UNRESOLVED('leagueStats.shrinkage')` 해소 — 전수 열거 분포의 분산 관측 후
...
|---|---|
| **담당** | `ui-builder` |
| **참조** | MASTER Part 9-5 · Part 11 + `docs/ONDOLOG_DESIGN.md` §13-5 |
| **산출물** | `app/(tabs)/magazine.tsx` 확장 |
| **완료 기준** | 다음 호 실루엣 + 제작 진행률 화면 동작. DESIGN §17 검증 5개 0건 |

**왜 독립 Phase인가** — Phase 4의 매거진 탭은 당시 규격(서재 격자)대로 정확히
완료됐고, 이후 규격이 늘어난 것이다. Phase 4를 되돌리면 이력이 왜곡된다.
Phase 8(PDF 뷰어)에 묶지 않는 이유는 성격이 다르기 때문이다 — 제작 탭은
발행물을 **보는 곳이 아니라 입력 창구**다.

**선행 관계** — 이 Phase가 §7 Phase 2 확장의 선행 조건이다. 참여형 코너
3종(셋로그·인터뷰·특별 게스트)이 이 탭을 **유일한 입력 창구**로 삼는다.

**Phase 6과의 관계** — 얼굴 인식 미탐으로 누락된 사진을 사람이 되살리는
창구도 이 탭이다(MASTER Part 9-4 매칭 판정 정책). 그전까지 미탐은 복구 수단이 없다.
...
| 상시 | 80문항 워딩·톤 최종 다듬기 | 콘텐츠 |
| Phase 6 전 | ~~생체정보 동의 UI~~ (완료). **법무 문구만 잔존** | 법무 (필수) |
| Phase 6 전 | ~~얼굴 인식 라이브러리 선정~~ → 완료 (1차: react-native-expo-facial-recognition) | 기술 |
| Phase 6 중 | **얼굴 매칭 임계값 실측 확정** — 오탐 회피 우선 확정(MASTER Part 9-4). 값은 `UNRESOLVED('faceMatch.threshold')`. 온디바이스 수기 측정, 메인 세션 작업 | 기술 |
| Phase 7 전 | ~~AI 매거진 메이커 캐릭터~~ → **폐기** | 콘텐츠 |
| Phase 7 전 | LLM 한국어 품질 실측 비교 | 기술 |
| Phase 7 전 | 채팅 데이터 AI 활용 고지 문구 | 법무 (필수) |
| ~~Phase 7 전~~ | ~~연애 온도 결합 공식~~ → **해소** (MASTER Part 10-7) | 기술 |
| ~~Phase 7 전~~ | ~~OVR 포지션 가중치 + 규준집단~~ → **해소** (MASTER Part 10-8) | 기술 |
| ~~Phase 7 전~~ | ~~베이지안 사전평균~~ → **해소** (전수 열거 분포) | 기술 |
| Phase 7 전 | **DNA base_score 궁합 공식** (Phase 2 자리표시자) — 10-6 범주 → 점수 변환 규칙 | 기술 |
| Phase 7 전 | `UNRESOLVED('leagueStats.shrinkage')` — 수축 강도. 전수 열거 분산 관측 후 | 기술 |
| Phase 7 전 | `UNRESOLVED('temperature.activityScore')` — 하루치 활동 점수 정의. 실사용 데이터 후 | 기술 |
| Phase 8 전 | RN PDF 뷰어 라이브러리 선정 | 기술 |
| Phase 8 전 | 매호 표지 자동 생성 로직 | 기술 |
| Phase 9 전 | 약관 · 개인정보 처리방침 | 법무 (필수) |
| Phase 9 전 | 발행 매거진 = 공동 저작물 조항 명시 | 법무 |
| Phase 2 확장 전 | 인터뷰 고정 질문 풀 / 특별 게스트 고정 질문 | 콘텐츠 |
| Phase 2 확장 전 | 특별 게스트 외부인 응답 웹 페이지 호스팅 | 기술 |
...
> ⚠️ **선행 조건: Phase 10(매거진 제작 탭)이 완료되어야 착수 가능하다.**
> 참여형 코너 3종(셋로그·인터뷰·특별 게스트)이 제작 탭을 유일한 입력 창구로 삼는다.

MVP 9개 항목이 검증된 뒤 착수한다. **기획은 완료, 구현은 미착수.**

| 코너 | 기획 | 스키마 | content 구조 |
|---|---|---|---|
| 우리 사이 인터뷰 | ✅ MASTER 17-6 | ✅ SCHEMA 9-B | ❌ |
| 오프라인 셋로그 | ✅ MASTER 17-7 | ✅ `entry_type` 확장 | ❌ |
| 특별 게스트 | ✅ MASTER 17-8 | ✅ SCHEMA 9-B | ❌ |
| 얼렁뚱땅 어림짐작 | ✅ MASTER 17-9 | 신규 테이블 불필요 | ❌ |

**착수 순서 제안**

| 순서 | 코너 | 이유 |
|---|---|---|
| 1 | 얼렁뚱땅 어림짐작 | 새 테이블 없음, LLM 호출 거의 없음, `engine-dev` 단독 |
| 2 | 오프라인 셋로그 | `entry_type` 추가만. 매거진 제작 탭 입력 구조 검증 |
| 3 | 우리 사이 인터뷰 | 신규 테이블 3종 + 참여 플로우 |
| 4 | 특별 게스트 | **외부 웹 페이지 필요** — 인프라가 늘어남 |

난이도와 인프라 부담이 낮은 순서다. 4번은 앱 밖 웹 호스팅이 처음 등장하는 지점이라 마지막에 둔다.

**선행 조건**: 매거진 제작 탭(MASTER Part 9-5)이 구현되어 있어야 한다. 인터뷰·셋로그의 유일한 입력 창구다.