...
`FORBIDDEN_KEYS` 목록의 원본은 `docs/ONDOLOG_MASTER.md` **Part 17-0-4**다.
코드는 그 목록을 상수로 갖는다. 여기에 목록을 복제하지 않는다 —
산문이 두 곳에 있으면 반드시 어긋난다.
...
> **2026-08 갱신**: 기존 7종 중 4종(오프라인 셋로그·우리 사이 인터뷰·특별 게스트·얼렁뚱땅 어림짐작)의 **기획이 완료**되었다(MASTER Part 17-6~17-9). 다만 이 문서의 **jsonb 구조는 아직 정의하지 않았다** — Phase 2 확장 착수 시점에 §2~§7과 같은 형식으로 추가한다.
>
> **생성 코드 작성 금지 원칙은 그대로 유지된다.** 기획이 있다는 것과 content 스키마가 있다는 것은 다르다. 이 문서에 구조가 정의되기 전까지 `corner-pipeline`은 해당 코너의 생성 코드를 작성하지 않는다.

| corner_type | 배치 | 기획 | content 구조 |
|---|---|---|---|
| `offline_setlog` | 주간 | ✅ Part 17-7 | ❌ 미정의 |
| `couple_interview` | 주간 | ✅ Part 17-6 | ❌ 미정의 |
| `special_guest` | 월간 | ✅ Part 17-8 | ❌ 미정의 |
| `rough_guess` | 월간 | ✅ Part 17-9 | ❌ 미정의 |
| `over_shoulder` | 월간 | ❌ 콜드스타트 정책 미정 | ❌ |
| `appendix` | 월간 | ❌ Year 2 | ❌ |
| `sponsored` | 월간 | ❌ Year 2 | ❌ |
...
| 코너 | 특이사항 |
|---|---|
| `couple_interview` | 문항별로 **각자의 답 + 상대 답 예측** 4개 값이 한 묶음. 정오 판정 필드를 만들지 않는다 |
| `offline_setlog` | 시간순 좌우 대칭. `left`/`right`가 아니라 `userId` 기준으로 담고 렌더러가 배치 |
| `special_guest` | 외부인 원문 + 닉네임. **승인된 답변만** content에 들어간다 |
| `rough_guess` | 8개 항목 중 7개는 규칙 기반이라 수치만. 7번(가장 말이 많았던 날)만 LLM 서술 |
